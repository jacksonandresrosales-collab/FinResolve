...
import { reclamosMock, ESTADOS, PRIORIDADES } from '@/lib/mockData';
import { PrioridadBadge, EstadoBadge, AlertaSLAIndicator } from '@/components/ui/Badges';
import { FileText, Filter, ChevronRight, Search, Plus } from 'lucide-react';
  ...
  const reclamosFiltrados = useMemo(() => {
    return reclamosMock.filter((r) => {
      ...
      const coincideBusqueda =
        ...
        r.identificacion.includes(busqueda);
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-slate-400">
          ...
          <span className="font-semibold text-white">{reclamosMock.length}</span> reclamos
        </p>
      </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-white/5">
                ...
                <th className="text-left py-3.5 px-4 text-xs font-semibold text-slate-400 uppercase tracking-wider hidden lg:table-cell">
                  Categoría
                </th>
                <th className="text-left py-3.5 px-4 text-xs font-semibold text-slate-400 uppercase tracking-wider">
                  Prioridad
                ...
                <th className="text-left py-3.5 px-4 text-xs font-semibold text-slate-400 uppercase tracking-wider hidden md:table-cell">
                  SLA
                </th>
                <th className="text-left py-3.5 px-4 text-xs font-semibold text-slate-400 uppercase tracking-wider hidden xl:table-cell">
                  Responsable
                </th>
                <th className="text-left py-3.5 px-4 text-xs font-semibold text-slate-400 uppercase tracking-wider hidden lg:table-cell">
                  Fecha
                ...
                >
                  ...
                  <td className="py-3.5 px-4">
                    <div>
                      ...
                      <p className="text-xs text-slate-500">{reclamo.identificacion}</p>
                    </div>
                  </td>
                  ...
                  <td className="py-3.5 px-4 hidden lg:table-cell">
                    <span className="text-sm text-slate-400">
                      {formatFecha(reclamo.fechaCreacion)}
                    </span>
                  </td>
                  <td className="py-3.5 px-4">
                    <Link
